...
def setup(
    ...
    **kw: Any,
    ...
    # The next major release replaces this backend with scikit-build-core's
    # setuptools plugin, which drops or changes several of these keywords. Warn
    # now (only when set to a non-default value) so projects can migrate first.
    if cmake_with_sdist:
        msg = (
            "cmake_with_sdist is deprecated and will be removed in the next major release; "
            "list sdist contents with a MANIFEST.in or setuptools-scm instead."
        )
        warnings.warn(msg, SKBuildDeprecationWarning, stacklevel=2)
    if cmake_install_target != "install":
        msg = (
            "cmake_install_target is deprecated and will be removed in the next major release; "
            "non-default install targets are not supported by the scikit-build-core backend."
        )
        warnings.warn(msg, SKBuildDeprecationWarning, stacklevel=2)
    if tuple(cmake_languages) != ("C", "CXX"):
        msg = (
            "cmake_languages is deprecated and will be ignored in the next major release; "
            "declare the languages in your CMakeLists.txt via project(... LANGUAGES ...) instead."
        )
        warnings.warn(msg, SKBuildDeprecationWarning, stacklevel=2)
    if cmake_minimum_required_version is not None:
        msg = (
            "cmake_minimum_required_version is deprecated and will be ignored in the next major release; "
            "set cmake.version in [tool.scikit-build] (or pin cmake in build-system.requires) instead."
        )
        warnings.warn(msg, SKBuildDeprecationWarning, stacklevel=2)
